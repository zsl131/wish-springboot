...
public class WeixinXmlTools {
     ...
     * @param fromUser 发送方
     * @param noticeList 文章列表
     * @param baseUrl 文章链接基础地址
     * @return
     */
    public static String buildSubscribeStr(String toUser, String fromUser, List<Notice> noticeList, String baseUrl) {
        if(noticeList==null || noticeList.size()<=0) {return createTextXml(toUser, fromUser, "系统正在全新改版升级中，敬请期待");}
        StringBuffer sb = new StringBuffer();
        sb.append("<xml>");
        sb.append("<ToUserName><![CDATA[").append(toUser).append("]]></ToUserName>").
                append("<FromUserName><![CDATA[").append(fromUser).append("]]></FromUserName>").
                ...
                append("<ArticleCount>"+noticeList.size()+"</ArticleCount>"). //TODO 这里设置文章条数
                append("<Articles>");
        ...
        for(Notice dto : noticeList) {
            sb.append("<item>").
                    ...
                    append("<Url><![CDATA[").append(baseUrl+"/weixin/notice/show?id="+dto.getId()).append("]]></Url>").
                    append("</item>");
        }
        sb.append("</Articles>");
        sb.append("</xml>");
